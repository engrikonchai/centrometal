 ...
 * "Dodaj u upit". Desktop puts the same pair inline in the detail column.
 */
export function ProductStickyBar({ product, locale }: { product: Product; locale: Locale }) {
  const dict = getDictionary(locale);

  return (
    <div className="sticky bottom-0 z-30 grid grid-cols-[1fr_1.4fr] gap-2.5 border-t-[0.5px] border-muted/30 bg-[rgba(249,249,251,0.88)] px-4 pb-[calc(10px+env(safe-area-inset-bottom))] pt-2.5 backdrop-blur-xl backdrop-saturate-[1.8] lg:hidden">
      <a
