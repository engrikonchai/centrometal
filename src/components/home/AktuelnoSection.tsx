 ...
 * redesign brief) — one image per category, keyed by the MNE slug.
 */
          ...
          {aktuelnoCategories.map((category, i) => (
            <Reveal key={category.slug.mne} delay={(i % 3) * 75}>
              <Link
                ...
                className="group relative block aspect-square w-full overflow-hidden rounded-lg transition duration-200 hover:-translate-y-0.5 hover:shadow-lg"
              >
                <Image
                  src={categoryImages[category.slug.mne]}
                  alt=""
                  fill
